...
template <class TCover> struct cover_cmp_by_new_t {
...
template <class TCover> struct cover_cmp_by_old_t {
  inline bool operator()(const TCover &x, const TCover &y) {
...
typedef struct hdiff_TCompress {
  ...
  int (*setParallelThreadNumber)(struct hdiff_TCompress *compressPlugin,
                                 int threadNum);
  // compress data to out_code; return compressed size, if error or not need
  // compress then return 0; if out_code->write() return
  // hdiff_stream_kCancelCompress(error) then return 0; if memory I/O can use
  // hdiff_compress_mem()
...
static hpatch_inline size_t
hdiff_compress_mem(const hdiff_TCompress *compressPlugin,
  ...
  hpatch_StreamPos_t codeLen =
      compressPlugin->compress(compressPlugin, &codeStream, &dataStream);
  if (codeLen != (size_t)codeLen)
    return 0; // error
...
struct IDiffSearchCoverListener {
  bool (*limitCover)(struct IDiffSearchCoverListener *listener,
                     ...
                     hpatch_StreamPos_t *out_leaveLen);
  void (*limitCover_front)(struct IDiffSearchCoverListener *listener,
                           const hpatch_TCover *front_cover,
                           hpatch_StreamPos_t *out_leaveLen);
};
struct IDiffResearchCover {
  void (*researchCover)(struct IDiffResearchCover *diffi,
                        struct IDiffSearchCoverListener *listener,
                        size_t limitCoverIndex,
                        hpatch_StreamPos_t sameCover_endPosBack,
                        hpatch_StreamPos_t hitPos, hpatch_StreamPos_t hitLen);
};
struct IDiffInsertCover {
  void *(*insertCover)(IDiffInsertCover *diffi, const void *pInsertCovers,
                       size_t insertCoverCount, bool insertIsCover32);
};
...
struct ICoverLinesListener {
  bool (*search_cover_limit)(ICoverLinesListener *listener, const void *pcovers,
                             size_t coverCount, bool isCover32);
  void (*research_cover)(ICoverLinesListener *listener,
                         IDiffResearchCover *diffi, const void *pcovers,
                         size_t coverCount, bool isCover32);
  void (*insert_cover)(ICoverLinesListener *listener, IDiffInsertCover *diffi,
                       void *pcovers, size_t coverCount, bool isCover32,
                       hpatch_StreamPos_t *newSize,
                       hpatch_StreamPos_t *oldSize);
  void (*search_cover_finish)(ICoverLinesListener *listener, void *pcovers,
                              size_t *pcoverCount, bool isCover32,
                              hpatch_StreamPos_t *newSize,
                              hpatch_StreamPos_t *oldSize);
  int (*get_max_match_deep)(
      const ICoverLinesListener
          *listener); // if null, default kDefaultMaxMatchDeepForLimit
  // *search_block* for muti-thread parallel match,can null
  void (*begin_search_block)(ICoverLinesListener *listener,
                             hpatch_StreamPos_t newSize, size_t searchBlockSize,
                             size_t kPartPepeatSize);
  hpatch_BOOL (*next_search_block_MT)(
      ICoverLinesListener *listener,
      hdiff_TRange *out_newRange); // must thread safe
};

struct hdiff_TMTSets_s { // used by $hdiff -s
  size_t threadNum;
  size_t threadNumForSearch; // NOTE: muti-thread search need frequent random
                             // disk read
