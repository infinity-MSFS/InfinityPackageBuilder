...
#define _HDIFFPATCH_VERSION                                                    \
  HDIFFPATCH_VERSION_MAJOR.HDIFFPATCH_VERSION_MINOR.HDIFFPATCH_VERSION_RELEASE
#define _HDIFFPATCH_QUOTE(str) #str
#define _HDIFFPATCH_EXPAND_AND_QUOTE(str) _HDIFFPATCH_QUOTE(str)
#define HDIFFPATCH_VERSION_STRING                                              \
  _HDIFFPATCH_EXPAND_AND_QUOTE(_HDIFFPATCH_VERSION)
#define HDIFFPATCH_VERSION_NUMBER                                              \
  ((HDIFFPATCH_VERSION_MAJOR * 1000 + HDIFFPATCH_VERSION_MINOR) * 1000 +       \
   HDIFFPATCH_VERSION_RELEASE)
...
#define _hpatch_align_type_lower(uint_type, p, align2pow)                      \
  (((uint_type)(p)) & (~(uint_type)((align2pow)-1)))
#define _hpatch_align_lower(p, align2pow)                                      \
  _hpatch_align_type_lower(hpatch_size_t, p, align2pow)
#define _hpatch_align_upper(p, align2pow)                                      \
  _hpatch_align_lower(((hpatch_size_t)(p)) + ((align2pow)-1), align2pow)
...
typedef struct hpatch_TStreamInput {
  ...
  // read() must read (out_data_end-out_data), otherwise error return
  // hpatch_FALSE
  hpatch_BOOL (*read)(const struct hpatch_TStreamInput *stream,
                      hpatch_StreamPos_t readFromPos, unsigned char *out_data,
...
typedef struct hpatch_TStreamOutput {
  void *streamImport;
  hpatch_StreamPos_t
      streamSize; // stream size,max writable range; not is write pos!
  // read_writed for ReadWriteIO, can null!
  hpatch_BOOL (*read_writed)(const struct hpatch_TStreamOutput *stream,
                             hpatch_StreamPos_t readFromPos,
                             unsigned char *out_data,
                             unsigned char *out_data_end);
  // write() must wrote (out_data_end-out_data), otherwise error return
  // hpatch_FALSE
  hpatch_BOOL (*write)(const struct hpatch_TStreamOutput *stream,
                       hpatch_StreamPos_t writeToPos, const unsigned char *data,
...
typedef struct hpatch_TDecompress {
  ...
  // error return 0.
  hpatch_decompressHandle (*open)(
      struct hpatch_TDecompress *decompressPlugin, hpatch_StreamPos_t dataSize,
  ...
  // decompress_part() must out (out_part_data_end-out_part_data), otherwise
  // error return hpatch_FALSE
  ...
  volatile hpatch_dec_error_t
      decError; // if you used decError value, once patch must used it's own
                // hpatch_TDecompress
} hpatch_TDecompress;
...
mem_as_hStreamOutput(hpatch_TStreamOutput *out_stream, unsigned char *mem,
...
// clip srcStream from clipBeginPos to clipEndPos as a new StreamInput;
void TStreamOutputClip_init(TStreamOutputClip *self,
                            const hpatch_TStreamOutput *srcStream,
                            hpatch_StreamPos_t clipBeginPos,
                            hpatch_StreamPos_t clipEndPos);

#define hpatch_kMaxPackedUIntBytes                                             \
  ((sizeof(hpatch_StreamPos_t) * 8 + 6) / 7 + 1)
hpatch_BOOL hpatch_packUIntWithTag(unsigned char **out_code,
                                   unsigned char *out_code_end,
                                   hpatch_StreamPos_t uValue,
                                   hpatch_uint highTag,
                                   const hpatch_uint kTagBit);
hpatch_uint hpatch_packUIntWithTag_size(hpatch_StreamPos_t uValue,
                                        const hpatch_uint kTagBit);
...
hpatch_BOOL hpatch_unpackUIntWithTag(const unsigned char **src_code,
                                     const unsigned char *src_code_end,
...
typedef struct hpatch_TCovers {
  ...
  hpatch_BOOL (*read_cover)(struct hpatch_TCovers *covers,
...
typedef struct hpatch_TOutputCovers {
  hpatch_BOOL (*push_cover)(struct hpatch_TOutputCovers *out_covers,
                            const hpatch_TCover *cover);
  void (*collate_covers)(
      struct hpatch_TOutputCovers
          *out_covers); // for support search covers by muti-thread
...
hpatch_inline static void _singleDiffInfoToHDiffInfo(
    hpatch_compressedDiffInfo *out_diffInfo,
    const hpatch_singleCompressedDiffInfo *singleDiffInfo) {
  ...
  memcpy(out_diffInfo->compressType, singleDiffInfo->compressType,
         strlen(singleDiffInfo->compressType) + 1);
}

typedef struct sspatch_listener_t {
  void *import;
  hpatch_BOOL (*onDiffInfo)(
      struct sspatch_listener_t *listener,
      const hpatch_singleCompressedDiffInfo *info,
      hpatch_TDecompress *
          *out_decompressPlugin, // find decompressPlugin by info->compressType
      unsigned char **out_temp_cache, //*out_temp_cacheEnd-*out_temp_cache ==
                                      // info->stepMemSize + (I/O cache memory)
      unsigned char **out_temp_cacheEnd); //  note: (I/O cache memory) >=
                                          //  hpatch_kStreamCacheSize*3
  void (*onPatchFinish)(
      struct sspatch_listener_t *listener, // onPatchFinish can null
...
typedef struct sspatch_coversListener_t {
  void *import;
  void (*onStepCoversReset)(
      struct sspatch_coversListener_t *listener,
      hpatch_StreamPos_t
          leaveCoverCount); // can NULL, data(in covers_cache) will invalid
...
hpatch_inline static void sspatch_covers_init(sspatch_covers_t *self) {
  memset(self, 0, sizeof(*self));
}
hpatch_inline static void
sspatch_covers_setCoversCache(sspatch_covers_t *self,
                              const unsigned char *covers_cache,
                              const unsigned char *covers_cacheEnd) {
...
sspatch_covers_isHaveNextCover(const sspatch_covers_t *self) {
