	...
	bool UpdateWindowManualResize(ImGuiWindow *window, ImVec2 &newSize, ImVec2 &newPosition) {
		...
		auto CalcWindowAutoFitSize = [CalcWindowSizeAfterConstraint](ImGuiWindow *window, const ImVec2 &size_contents) {
			...
			if (window->Flags & ImGuiWindowFlags_Tooltip) {
				...
				ImVec2 size_auto_fit = ImClamp(size_desired, size_min, ImMax(size_min, {
		...
		const bool clip_with_viewport_rect = !(g.IO.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport) || (g.IO.MouseHoveredViewport != window->ViewportId) || !(
