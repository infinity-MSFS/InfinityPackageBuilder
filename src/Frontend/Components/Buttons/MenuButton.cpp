...
namespace Infinity {
    ...
    bool MenuButton::RenderButton(float x, float y, float width, float height, const char *label, int to, bool settings) {
        ...
        if (m_CurrentActiveID == to) {
            if (settings) {
                window->DrawList->AddRectFilled(rct.Min, rct.Max, ImColor(0.5f, 0.5f, 1.0f, 0.5f), 8, 0);
                ImGui::PushStyleColor(ImGuiCol_Text, {1.0f, 1.0f, 1.0f, 1.0f});
            } else {
                window->DrawList->AddRectFilled(rct.Min, rct.Max, ImColor(1.0f, 1.0f, 1.0f, 0.7f), 8, 0);
        ...
        if (held || hovered) {
            if (settings) {
                window->DrawList->AddRect(rct.Min, rct.Max, ImColor(0.5f, 0.5f, 1.0f, 0.5f), 8, 0, 2.0f);
            } else {
                window->DrawList->AddRect(rct.Min, rct.Max, ImColor(1.0f, 1.0f, 1.0f, 0.7f), 8, 0, 2.0f);
            }

        } else {
            if (settings) {
                window->DrawList->AddRect(rct.Min, rct.Max, ImColor(0.5f, 0.5f, 1.0f, 0.7f), 8, 0, 2.0f);
            } else {
                window->DrawList->AddRect(rct.Min, rct.Max, ImColor(1.0f, 1.0f, 1.0f, 1.0f), 8, 0, 2.0f);
            }
        }
