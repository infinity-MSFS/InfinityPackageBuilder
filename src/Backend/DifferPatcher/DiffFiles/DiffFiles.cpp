...
namespace Infinity {
    ...
    std::string DiffFiles::TrimRelativePath(const std::string &relative_path) {
        size_t last_slash_index = relative_path.find_last_of('\\');
    ...
    std::vector<uint8_t> DiffFiles::Diff(const std::string &file_a, const std::string &file_b, const std::string &temp_directory) {
        if (file_a.size() > 4 && file_b.size() > 4 && file_a.substr(file_a.size() - 4) == ".dds" && file_b.substr(file_b.size() - 4) == ".dds") {
            ...
            std::string patch_file_name = temp_directory + "/" + gen_random(10) + ".infinitypatch";
            ...
            std::vector<uint8_t> patch_data((std::istreambuf_iterator<char>(patch_file)), std::istreambuf_iterator<char>());
    ...
    void DiffFiles::SpawnDiffTask(const std::string &aircraft_folder_a, const std::string &aircraft_folder_b, const std::string &output_path, const std::string &relative_path,
        ...
        std::string file_path_a = aircraft_folder_a + "/" + relative_path;
        std::string file_path_b = aircraft_folder_b + "/" + relative_path;

        auto start_time = std::chrono::high_resolution_clock::now();

        std::string output_file = output_path + "/" + TrimRelativePath(relative_path);

        std::vector<uint8_t> patch = Diff(file_path_a, file_path_b, temp_directory);
        std::ofstream out(output_file, std::ios::binary);
        out.write(reinterpret_cast<const char *>(patch.data()), patch.size());

        auto elapsed_time = std::chrono::high_resolution_clock::now() - start_time;
        std::chrono::duration<double> duration = elapsed_time;
    ...
    void DiffFiles::DiffFromMap(const PatchMapStructure &map_structure, const std::string &aircraft_folder_a, const std::string &aircraft_folder_b, const std::string &output_path,
        ...
        for (const auto &relative_path: map_structure.changed_files) {
            diff_threads.push_back(std::thread([=] { SpawnDiffTask(aircraft_folder_a, aircraft_folder_b, output_path, relative_path, temp_directory); }));
        }
