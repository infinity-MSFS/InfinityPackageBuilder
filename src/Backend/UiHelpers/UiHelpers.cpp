...
namespace Infinity::UI {
    ...
    ImRect RectOffset(const ImRect &rect, const ImVec2 xy) { return RectOffset(rect, xy.x, xy.y); }

    void DrawButtonImage(const std::shared_ptr<Image> &imageNormal, const std::shared_ptr<Image> &imageHovered, const std::shared_ptr<Image> &imagePressed, const ImU32 tintNormal,
                         const ImU32 tintHovered, const ImU32 tintPressed, const ImVec2 rectMin, const ImVec2 rectMax) {
    ...
    void DrawButtonImage(const std::shared_ptr<Image> &imageNormal, const std::shared_ptr<Image> &imageHovered, const std::shared_ptr<Image> &imagePressed, const ImU32 tintNormal,
                         const ImU32 tintHovered, const ImU32 tintPressed, const ImRect rectangle) {
        DrawButtonImage(imageNormal, imageHovered, imagePressed, tintNormal, tintHovered, tintPressed, rectangle.Min, rectangle.Max);
    };

    void DrawButtonImage(const std::shared_ptr<Image> &image, const ImU32 tintNormal, const ImU32 tintHovered, const ImU32 tintPressed, const ImVec2 rectMin, const ImVec2 rectMax) {
        DrawButtonImage(image, image, image, tintNormal, tintHovered, tintPressed, rectMin, rectMax);
    };

    void DrawButtonImage(const std::shared_ptr<Image> &image, const ImU32 tintNormal, const ImU32 tintHovered, const ImU32 tintPressed, const ImRect rectangle) {
        DrawButtonImage(image, image, image, tintNormal, tintHovered, tintPressed, rectangle.Min, rectangle.Max);
    };


    void DrawButtonImage(const std::shared_ptr<Image> &imageNormal, const std::shared_ptr<Image> &imageHovered, const std::shared_ptr<Image> &imagePressed, const ImU32 tintNormal,
                         const ImU32 tintHovered, const ImU32 tintPressed) {
    ...
    void DrawButtonImage(const std::shared_ptr<Image> &image, const ImU32 tintNormal, const ImU32 tintHovered, const ImU32 tintPressed) {
    ...
    void RenderWindowOuterBorders(ImGuiWindow *window) {
        ...
        auto GetResizeBorderRect = [](ImGuiWindow *window, const int border_n, const float perp_padding, const float thickness) {
    ...
    bool UpdateWindowManualResize(ImGuiWindow *window, ImVec2 &newSize, ImVec2 &newPosition) {
        ...
        auto CalcWindowAutoFitSize = [CalcWindowSizeAfterConstraint](ImGuiWindow *window, const ImVec2 &size_contents) {
            ...
            if (window->Flags & ImGuiWindowFlags_Tooltip) {
                ...
                if (is_popup || is_menu)
                    // Popups and menus bypass style.WindowMinSize by default, but we give then a non-zero minimum size
                    // to facilitate understanding problematic cases (e.g. empty popups)
                    size_min = ImMin(size_min, ImVec2(4.0f, 4.0f));

                // FIXME-VIEWPORT-WORKAREA: May want to use GetWorkSize() instead of Size depending on the type of
                // windows?
                ...
                // When the window cannot fit all contents (either because of constraints, either because screen is too
                // small), we are growing the size on the other axis to compensate for expected scrollbar. FIXME: Might
                // turn bigger than ViewportSize-WindowPadding.
                ...
                bool will_have_scrollbar_y = (size_auto_fit_after_constraint.y - size_pad.y - decoration_up_height < size_contents.y && !(window->Flags & ImGuiWindowFlags_NoScrollbar)) ||
        ...
        static const float WINDOWS_HOVER_PADDING = 4.0f; // Extend outside window for hovering/resizing (maxxed with TouchPadding) and inside windows for
                                                         // borders. Affect FindHoveredWindow().
        ...
        //   This is however not the case with current backends under Win32, but a custom borderless window
        ...
        const bool clip_with_viewport_rect =
                !(g.IO.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport) || (g.IO.MouseHoveredViewport != window->ViewportId) || !(window->Viewport->Flags & ImGuiViewportFlags_NoDecoration);
        ...
        for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
            ...
            // Using the FlattenChilds button flag we make the resize button accessible even if we are hovering over a
            // child window
            bool hovered, held;
            ...
            if (held && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
                ...
                // We don't use an incremental MouseDelta but rather compute an absolute target size based on mouse
                // position
    ...
    bool BeginMenubar(const ImRect &barRectangle) {
        ...
        // with window full rect. We remove 1 worth of rounding to Max.x to that text in long menus and small windows
        // don't tend to display over the lower-right rounded area, which looks particularly glitchy.
        ...
        // We overwrite CursorMaxPos because BeginGroup sets it to CursorPos (essentially the .EmitItem hack in
        // EndMenuBar() would need something analogous here, maybe a BeginGroupEx() with flags).
    ...
    void EndMenubar() {
        ...
        ImGui::PopClipRect();
        ImGui::PopID();
        window->DC.MenuBarOffset.x = window->DC.CursorPos.x - window->Pos.x; // Save horizontal position so next append can reuse it. This is
